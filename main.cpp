...
#include "arquivo.h"
//#include "valor_conta.h" n�o sera utilizado at� o momento
...
int main(int argc, char *argv[])
{
	...
	switch(op) {
		...
		case 1: 
				...
				aux = uti.gera_quarto(mapPessoa, itP); //O VALOR RETORNADO PELA FUN��O SERA O NUMERO DO QUARTO
				mapPessoa[aux] = pessoa(nome, diarias);
				
				diarias = valor_diaria * diarias ;
				mapConta.insert(make_pair(aux, diarias)); //O MAPCONTA DEVERA SEMPRE USAR O MAKE_PAIR NA PRIMEIRA INICIALIZA��O
		...
		case 2: cout << "Digite o numero do seu quarto: ";
				...
				case 5: cout <<" Identifique-se, com seu numero de quarto";
						...
						if(uti.verifica_quarto(quarto, mapPessoa, itP)){ //VERIFICA SE O QUARTO EXISTE
							cout<<" Reserva encontrada, aguarde"<<endl;
								for(itS = mapServico.begin(); itS != mapServico.end(); itS++){ // LISTA OS SERVI�OS
									cout<< itS->first ;
									itS->second.imprimir() ;
								}
							cout <<" escolha o servi�o"<<endl ;
							cin	 >> servico ;
							itS = mapServico.find(servico) ;
							
							itC = mapConta.find(quarto) ; 
							teste =itC->second + itS->second.getvalor() ;//SOMA O VALOR DO SERVI�O PRA SER ADICIONADO NA CONTA
							...
							cout<<" Sua reserva n�o existe"<<endl;
						}
				break;
				
					
				case 6: cout<<" servi�os disponiveis"<< endl;
						for(itS = mapServico.begin(); itS != mapServico.end(); itS++){ // LISTA OS SERVI�OS
									cout<< itS->first ;
									itS->second.imprimir() ;
								}
				...
				case 7: cout<<" identifique-se com o numero do quarto"<< endl;
						...
						if(uti.verifica_quarto(quarto, mapPessoa, itP)){
							...
							cout<<" voce n�o tem reservas"<<endl;
				break;
